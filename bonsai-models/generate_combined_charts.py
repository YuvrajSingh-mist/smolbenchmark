...
Charts saved to non-reasoning-models/artifacts/charts/ (used by benchmark_report.md)
"""
...
BONSAI_ROOT = Path.home() / "Desktop/smolbenchmark/bonsai-models"
ARTIFACTS   = BONSAI_ROOT / "artifacts/llamacpp"

# Charts go into the report's artifact directory
OUT_DIR = Path.home() / "Desktop/smolbenchmark/non-reasoning-models/artifacts/charts"
    ...
    for model in MODELS:
        for gen in GEN_LENGTHS:
            for ctx in PROMPT_LENGTHS:
                ...
                rl_avg   = pct("request_latency", "p50")
                ...
                # output tok/J = output tokens per joule of decode energy only (prefill excluded)
...
print("\n=== CANONICAL CELL (ctx=2048, gen=256) ===")
...
for model in MODELS:
    for mode in ["7W", "15W", "25W", "MAXN"]:
        ...
        print(f"  {model:22s} {mode:4s}  "
              ...
              f"power={row.power_w:5.2f}W  rl={row.rl_avg:8.1f}ms")
...
toks_mean = df.groupby(["model", "mode"])["tok_s"].mean().unstack("mode").reindex(MODELS)
for model in MODELS:
...
best_total = df.groupby(["model", "mode", "prompt", "gen"])["total_tokj"].mean()
for model in MODELS:
    try:
...
# 1. Output Tok/s vs Prompt: 2×3 facet, gen=256
faceted_line_chart(df, "tok_s", "Output Tok/s", 256, "1_tok_s_vs_prompt_gen256.png")

# 2. Output Tok/J vs Prompt: 2×3 facet, gen=256
...
for m in MODELS:
    ...
    for mode in ["7W", "15W", "25W", "MAXN"]:
        sub = df[(df.model == m) & (df["mode"] == mode) &
                 (df.prompt == CANONICAL_CTX) & (df.gen == CANONICAL_GEN)]
        row_vals.append(sub["itl"].mean() if not sub.empty else np.nan)
...
for mode_name in modes_avail:
    ...
    if sub.empty:
        continue
    pivot = sub.pivot_table(index="gen", columns="prompt", values="itl", aggfunc="mean")
...
for mode_name in modes_avail:
    ...
    if sub.empty:
        continue
    pivot = sub.pivot_table(index="gen", columns="prompt", values="ttft", aggfunc="mean")
    fig, ax = plt.subplots(figsize=(7, 4))
...
for mode_name in modes_avail:
    ...
    if sub.empty:
        continue
    pivot = sub.pivot_table(index="gen", columns="prompt", values="prefill", aggfunc="mean")
    fig, ax = plt.subplots(figsize=(7, 4))
...
# ── Appendix E: tok/s and tok/J heatmaps at each power mode ───────────────────
print("\n── Appendix E charts ──")
...
for gen_val in GEN_LENGTHS:
    ...
    faceted_line_chart(df, "rl_avg", "Request latency (ms)", gen_val,
...
for gen_val in GEN_LENGTHS:
    ...
    if gen_val == 256:
        faceted_line_chart(df, "decode_tokj", "Decode tok/J", 256,
                           "22f_decode_tokj_vs_prompt_gen256.png")
...
faceted_line_chart(df, "rl_avg",    "Request latency (ms)", 256,
